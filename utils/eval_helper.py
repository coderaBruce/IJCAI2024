...
def evaluate(config_dict, model, data_reader, metrics, mode, normalize = False):

    
    model.eval()

    UU, II = model.get_embed()
    ...
    with torch.no_grad():
        ...
        if normalize:
            user_vecs = F.normalize(UU, dim = -1).cpu().numpy()
            item_vecs = F.normalize(II, dim = -1).cpu().numpy()
        else:
            user_vecs = UU.cpu().numpy()
            item_vecs = II.cpu().numpy()
        ...
        if mode == 'valid':
            query_indexes = np.array(list(data_reader.valid_u_dict.keys()))
        
            user_vecs = user_vecs[query_indexes]
        

            res = evaluate_metrics(user_vecs, 
                                item_vecs, 
                                data_reader.train_u_dict, 
                                data_reader.valid_u_dict, 
                                query_indexes,
                                metrics,
                                parallel = True)
        else:
            query_indexes = np.array(list(data_reader.test_u_dict.keys()))
